...
CREATE OR REPLACE FUNCTION development.return_personen(
...
SELECT (json_agg(row_to_json(rel))) AS row_value
    FROM development.obj_basis AS obb
        JOIN development.rel_personen AS rel ON rel.ref_objekt_id = obb.objekt_id
        JOIN development.def_personen AS def ON rel.ref_funktion_id = def.id
